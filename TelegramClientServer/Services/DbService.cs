...
namespace TelegramLib.Services
{
    ...
    {
        //Partly getting values contsts 
        private const int _getContactsAmount = 15000;
        private const int _getChatsAmount = 15000;
        private const int _getMessagesAmount = 15000;


        public static TelSystem GetPartlyTelSystem(int userId)
        {
            mainClass.User user = GetUserById(userId);// GetUserByLoginAndPassword(login, password);
            if (user is null) return null;
            TelSystem system = new TelSystem();

            system.LoggedUser = user;
            user.BlockedUsers = GetBlockedContactsByUserId(userId);
            system.Settings = GetSettingsByUserId(userId);
            system.Folders = GetFoldersByUserId(userId);

            system.Contacts = GetPartOfTheContacts(userId, 0);
            system.Chats = GetPartOfTheUserChat(userId, 0); //GetUserChatsByUserId(userId);
            system.SavedMesesChat = GetSavedMessageChat(userId);
        ...
        private static List<mainClass.FolderObjs.Folder> GetFoldersByUserId(int userId)
        {
            using (var model = new TelegramModel())
            {
                ...
                if (!folders.Any()) return new List<mainClass.FolderObjs.Folder>();

                var folderIds = folders.Select(x => x.Id).ToHashSet();
                var iconIds = folders.Where(x => x.IconId != null).Select(x => (int)x.IconId).ToHashSet();

                var icons = model.FolderIcons
                    .Where(x => iconIds.Contains(x.Id))
                    .ToDictionary(x => x.Id, x => x.Name);

                var allFolderContacts = model.ContactsInFolder
                    .Where(x => folderIds.Contains((int)x.FolderId))
                    .ToList();

                var contactUserIds = allFolderContacts
                    .Where(x => x.ContactId != null)
                    .Select(x => (int)x.ContactId)
                    .ToHashSet();

                var users = model.User
                    .Where(x => contactUserIds.Contains(x.Id))
                    .ToList();

                var colors = model.UserColor
                    .Where(x => contactUserIds.Contains((int)x.UserId))
                    .ToDictionary(x => (int)x.UserId);

                var images = model.UserImage
                    .Where(x => contactUserIds.Contains((int)x.UserId))
                    .GroupBy(x => (int)x.UserId)
                    .ToDictionary(g => g.Key, g => g.OrderByDescending(i => i.AddDate).ToList());

                var userDict = users.ToDictionary(u => u.Id, u => new mainClass.User
                {
                    Id = u.Id,
                    ...
                    BirthDay = u.Birthday,
                    LastSeenOnline = u.LastOnline ?? DateTime.Now,
                    MainColor = colors.TryGetValue(u.Id, out var c)
                        ? new ColorHelper(u.Id, (byte)c.R, (byte)c.G, (byte)c.B)
                        : new ColorHelper(u.Id),
                    UserImages = images.TryGetValue(u.Id, out var imgs)
                        ? imgs.Select(i => new mainClass.UserParams.UserImage { Name = i.Name, Date = (DateTime)i.AddDate }).ToList()
                        : new List<mainClass.UserParams.UserImage>()
                });

                var contactsByFolder = allFolderContacts
                    .GroupBy(x => (int)x.FolderId)
                    .ToDictionary(
                        g => g.Key,
                        g => g.ToList()
                    );

                return folders.Select(f => new mainClass.FolderObjs.Folder
                {
                    Id = f.Id,
                    Name = f.Name,
                    IconName = icons.TryGetValue((int)f.IconId, out var iconName) ? iconName : string.Empty,
                    Contacts = contactsByFolder.TryGetValue(f.Id, out var contacts)
                        ? contacts.Where(x => !(bool)x.IsExclude && x.ContactId != null && userDict.ContainsKey((int)x.ContactId))
                                  .Select(x => userDict[(int)x.ContactId]).ToList()
                        : new List<mainClass.User>(),
                    ExcludedContacts = contactsByFolder.TryGetValue(f.Id, out var excluded)
                        ? excluded.Where(x => (bool)x.IsExclude && x.ContactId != null && userDict.ContainsKey((int)x.ContactId))
                                  .Select(x => userDict[(int)x.ContactId]).ToList()
                        : new List<mainClass.User>()
                }).ToList();
            }
        ...
        //Partly contacts
        public static List<UserContactcs> GetPartOfTheContacts(int userId, int tempContactId)
        {
            List<UserContactcs> res = new List<UserContactcs>();

            using (var model = new TelegramModel())
            {
                List<Contacts> tempContacts = model.Contacts.Where(x => x.UserId == userId && x.Id > tempContactId).Take(_getContactsAmount).ToList();

                foreach (var val in tempContacts)
                {
                    UserContactcs toAdd = GetContactById(val.Id, userId);
                    if (toAdd is null) continue;
        ...
        public static List<UserChat> GetPartOfTheUserChat(int userId, int tempChatId)
        {
            List<UserChat> res = new List<UserChat>();
            using (var model = new TelegramModel())
            {
                List<Chat> tempChat = model.Chat
                    .Where(x => x.UserId == userId && x.Id > tempChatId && !x.IsSavedMessagesChat)
                    .Take(_getChatsAmount)
                    .ToList();

                if (!tempChat.Any()) return res;

                var chatIds = tempChat.Select(x => x.Id).ToHashSet();
                var chatterIds = tempChat.Where(x => x.ChatterId != null).Select(x => (int)x.ChatterId).ToHashSet();
                var autoDelIds = tempChat.Where(x => x.AutoDeleteId != null).Select(x => (int)x.AutoDeleteId).ToHashSet();

                var chatterUsers = model.User.Where(x => chatterIds.Contains(x.Id)).ToList();
                
                var chatterColors = model.UserColor.Where(x => chatterIds.Contains((int)x.UserId)).ToDictionary(x => (int)x.UserId);

                var chatterImages = model.UserImage
                    .Where(x => chatterIds.Contains((int)x.UserId))
                    .GroupBy(x => (int)x.UserId)
                    .ToDictionary(g => g.Key, g => g.OrderByDescending(i => i.AddDate).ToList());
                var chatterMasks = model.ContactImageMask
                    .Where(x => x.UserId == userId && chatterIds.Contains((int)x.FriendId))
                    .ToDictionary(x => (int)x.FriendId);

                var chatterDict = chatterUsers.ToDictionary(u => u.Id, u =>
                {
                    var user = new mainClass.User
                    {
                        Id = u.Id,
                        IsOnline = (bool)u.IsOnline,
                        Login = u.Login,
                        ...
                        BirthDay = u.Birthday,
                        LastSeenOnline = u.LastOnline ?? DateTime.Now,
                        MainColor = chatterColors.TryGetValue(u.Id, out var c)
                            ? new ColorHelper(u.Id, (byte)c.R, (byte)c.G, (byte)c.B)
                            : new ColorHelper(u.Id),
                        UserImages = chatterImages.TryGetValue(u.Id, out var imgs)
                            ? imgs.Select(i => new mainClass.UserParams.UserImage { Name = i.Name, Date = (DateTime)i.AddDate }).ToList()
                            : new List<mainClass.UserParams.UserImage>()
                    };

                    if (chatterMasks.TryGetValue(u.Id, out var mask))
                    {
                        user.ImageMask = new mainClass.UserParams.UserImage(mask.ImageName, DateTime.Now);
                        user.UserImages.Insert(0, user.ImageMask);
                    }

                    return user;
                });

                var blockedUsers = GetBlockedContactsByUserId(userId);

                var autoDelTypes = model.AutoDeleteType
                    .Where(x => autoDelIds.Contains(x.Id))
                    .ToDictionary(x => x.Id);

                var allPossibleBgs = model.PossibleChatBGs
                    .Where(x => chatIds.Contains((int)x.ChatId))
                    .ToList();
                var allBgIds = allPossibleBgs.Where(x => x.ChatBgId != null).Select(x => (int)x.ChatBgId).ToHashSet();
                var allChatBgs = model.ChatBG.Where(x => allBgIds.Contains(x.Id)).ToDictionary(x => x.Id);
                var bgByChatId = allPossibleBgs
                    .GroupBy(x => (int)x.ChatId)
                    .ToDictionary(g => g.Key, g =>
                    {
                        var first = g.First();
                        if (first.ChatBgId == null || !allChatBgs.TryGetValue((int)first.ChatBgId, out var bg)) return null;
                        return new ChatBackground
                        {
                            FileName = bg.Name,
                            IsGeneral = false,
                            IsBlurred = first.IsBlurred ?? false
                        };
                    });

                var notifications = model.NotificationChats
                    .Where(x => chatIds.Contains((int)x.ChatId))
                    .ToDictionary(x => (int)x.ChatId, x => x.IsOn ?? false);

                var allMessages = model.Messages
                    .Where(x => chatIds.Contains((int)x.ChatId) && !x.IsInSchedule)
                    .OrderByDescending(x => x.Id)
                    .ToList();

                var allSchedMessages = model.Messages
                    .Where(x => chatIds.Contains((int)x.ChatId) && x.IsInSchedule)
                    .OrderByDescending(x => x.Id)
                    .ToList();

                var pinnedMessages = model.Messages
                    .Where(x => chatIds.Contains((int)x.ChatId) && x.IsPinned == true)
                    .ToList();

                var allMsgs = allMessages.Concat(allSchedMessages).Concat(pinnedMessages).ToList();

                var imageIds = allMsgs.Where(x => x.ImageId != null).Select(x => (int)x.ImageId).ToHashSet();
                var videoIds = allMsgs.Where(x => x.VideoId != null).Select(x => (int)x.VideoId).ToHashSet();
                var gifIds = allMsgs.Where(x => x.GifId != null).Select(x => (int)x.GifId).ToHashSet();
                var stickerIds = allMsgs.Where(x => x.StickerId != null).Select(x => (int)x.StickerId).ToHashSet();
                var shareIds = allMsgs.Where(x => x.ShareContactId != null).Select(x => (int)x.ShareContactId).ToHashSet();
                var msgAutoDelIds = allMsgs.Where(x => x.ChangedAutoDelId != null).Select(x => (int)x.ChangedAutoDelId).ToHashSet();

                var images = model.ChatImage.Where(x => imageIds.Contains(x.Id)).ToDictionary(x => x.Id, x => x.Name);
                var videos = model.MessageVideo.Where(x => videoIds.Contains(x.Id)).ToDictionary(x => x.Id, x => x.Name);
                var gifs = model.GIF.Where(x => gifIds.Contains(x.Id)).ToDictionary(x => x.Id, x => x.Name);
                var stickers = model.StickerImage.Where(x => stickerIds.Contains(x.Id)).ToDictionary(x => x.Id, x => x.Name);
                var shares = model.ShareContactMessage.Where(x => shareIds.Contains(x.Id)).ToDictionary(x => x.Id);
                var msgAutoDelTypes = model.AutoDeleteType.Where(x => msgAutoDelIds.Contains(x.Id)).ToDictionary(x => x.Id);
                var shareUserIds = shares.Values.Where(x => x.UserId != null).Select(x => (int)x.UserId).ToHashSet();
                var shareUsers = model.User.Where(x => shareUserIds.Contains(x.Id)).ToDictionary(x => x.Id);

                Message BuildMessage(Messages mes)
                {
                    Message toAdd;
                    if (mes.MessageRefference != null || mes.ChangedAutoDelId != null || mes.StatDate != null)
                        toAdd = new StaticMessage();
                    else if (mes.ShareContactId != null)
                        toAdd = new mainClass.Messages.ShareContactMessage();
                    else if (mes.Message == null)
                        toAdd = new MediaAction();
                    else
                        toAdd = new TextMessage();

                    toAdd.Id = mes.Id;
                    toAdd.SenderUserId = mes.SenderId ?? -1;
                    toAdd.RepliedQuote = mes.MessageQuote;
                    toAdd.SentTime = mes.SentDate ?? DateTime.Now;
                    toAdd.IsRead = mes.IsRead;
                    toAdd.IsPinned = mes.IsPinned ?? false;
                    toAdd.ForwardedFromId = mes.ForwardedFrom;

                    if (toAdd is MediaAction band) band.BandId = mes.BandId ?? -1;

                    if (toAdd is TextMessage textMsg)
                    {
                        textMsg.Text = mes.Message;
                        textMsg.IsEdited = mes.IsEdited;
                        textMsg.RepliedMessageId = mes.ReplyId;
                    }
                    else if (mes.ImageId != null && images.TryGetValue((int)mes.ImageId, out var imgName))
                    { ((MediaAction)toAdd).IsSticker = false; ((MediaAction)toAdd).MediaName = imgName; }
                    else if (mes.VideoId != null && videos.TryGetValue((int)mes.VideoId, out var vidName))
                    { ((MediaAction)toAdd).IsSticker = false; ((MediaAction)toAdd).MediaName = vidName; }
                    else if (mes.GifId != null && gifs.TryGetValue((int)mes.GifId, out var gifName))
                    { ((MediaAction)toAdd).IsSticker = false; ((MediaAction)toAdd).MediaName = gifName; }
                    else if (mes.StickerId != null && stickers.TryGetValue((int)mes.StickerId, out var stickerName))
                    { ((MediaAction)toAdd).IsSticker = true; ((MediaAction)toAdd).MediaName = stickerName; }

                    if (toAdd is mainClass.Messages.ShareContactMessage share && mes.ShareContactId != null
                        && shares.TryGetValue((int)mes.ShareContactId, out var shareModel))
                    {
                        share.SharedName = shareModel.Name;
                        if (shareModel.UserId != null && shareUsers.TryGetValue((int)shareModel.UserId, out var su))
                            share.SharedUser = new mainClass.User { Id = su.Id, Name = su.Name, Login = su.Login };
                    }

                    if (toAdd is StaticMessage statMsg)
                    {
                        statMsg.MessageReferenceId = mes.MessageRefference ?? -1;
                        statMsg.DelType = mes.ChangedAutoDelId != null && msgAutoDelTypes.TryGetValue((int)mes.ChangedAutoDelId, out var dt)
                            ? GetAutoDelTypeByTypeString(dt.Name)
                            : (Enums.Chat.AutoDeleteType?)null;
                        statMsg.Date = mes.StatDate;
                    }

                    return toAdd;
                }

                var msgsByChat = allMessages
                    .GroupBy(x => (int)x.ChatId)
                    .ToDictionary(g => g.Key, g => g.Take(_getMessagesAmount).OrderBy(x => x.Id).Select(BuildMessage).ToList());

                var schedByChat = allSchedMessages
                    .GroupBy(x => (int)x.ChatId)
                    .ToDictionary(g => g.Key, g => g.Take(_getMessagesAmount).OrderBy(x => x.Id).Select(BuildMessage).ToList());

                var pinnedByChat = pinnedMessages
                    .GroupBy(x => (int)x.ChatId)
                    .ToDictionary(g => g.Key, g => g.Select(BuildMessage).ToList());

                foreach (var chat in tempChat)
                {
                    if (chat.ChatterId is null) continue;

                    int chatterId = (int)chat.ChatterId;
                    if (!chatterDict.TryGetValue(chatterId, out var chatter)) continue;

                    chatter.BlockedUsers = blockedUsers;

                    Enums.Chat.AutoDeleteType autoDelType = Enums.Chat.AutoDeleteType.Nothing;
                    if (chat.AutoDeleteId != null && autoDelTypes.TryGetValue((int)chat.AutoDeleteId, out var adt))
                        autoDelType = GetAutoDelTypeByTypeString(adt.Name);

                    bgByChatId.TryGetValue(chat.Id, out var chatBg);
                    msgsByChat.TryGetValue(chat.Id, out var messages);
                    schedByChat.TryGetValue(chat.Id, out var schedMessages);
                    pinnedByChat.TryGetValue(chat.Id, out var pinned);

                    UserChat toAdd = new UserChat(
                        chat.Id,
                        chatter,
                        messages ?? new List<Message>(),
                        chatBg,
                        autoDelType,
                        pinned ?? new List<Message>(),
                        schedMessages ?? new List<Message>()
                    );

                    toAdd.NotificationStatus = notifications.TryGetValue(chat.Id, out var notif) && notif;
                    toAdd.IsPinned = chat.IsPinned ?? false;
                    toAdd.IsMarked = chat.IsRead ?? false;
        ...
        //Partly messages 
        public static List<Message> GetPartOfTheMessages(
            int chatId, int tempMessageId, bool isSchedule = false)
        {
            List<Message> res = new List<Message>();
            using (var model = new TelegramModel())
            {
                List<Messages> tempMessages = model.Messages
                    .Where(x => x.ChatId == chatId
                        && (tempMessageId == 0 || x.Id < tempMessageId)
                        && x.IsInSchedule == isSchedule)
                    .OrderByDescending(x => x.Id)
                    .Take(_getMessagesAmount)
                    .OrderBy(x => x.Id)
                    .ToList();

                var imageIds = tempMessages.Where(x => x.ImageId != null).Select(x => (int)x.ImageId).ToHashSet();
                var videoIds = tempMessages.Where(x => x.VideoId != null).Select(x => (int)x.VideoId).ToHashSet();
                var gifIds = tempMessages.Where(x => x.GifId != null).Select(x => (int)x.GifId).ToHashSet();
                var stickerIds = tempMessages.Where(x => x.StickerId != null).Select(x => (int)x.StickerId).ToHashSet();
                var shareIds = tempMessages.Where(x => x.ShareContactId != null).Select(x => (int)x.ShareContactId).ToHashSet();
                var autoDelIds = tempMessages.Where(x => x.ChangedAutoDelId != null).Select(x => (int)x.ChangedAutoDelId).ToHashSet();
                ...
                var stickers = model.StickerImage.Where(x => stickerIds.Contains(x.Id)).ToDictionary(x => x.Id, x => x.Name);
                var shares = model.ShareContactMessage.Where(x => shareIds.Contains(x.Id)).ToDictionary(x => x.Id);
                var autoDelTypes = model.AutoDeleteType.Where(x => autoDelIds.Contains(x.Id)).ToDictionary(x => x.Id);
                ...
                foreach (var mes in tempMessages)
                {
                    ...
                    if (toAdd is MediaAction band)
                        band.BandId = mes.BandId is null ? -1 : (int)mes.BandId;
                    ...
                    else if (mes.ImageId != null && images.TryGetValue((int)mes.ImageId, out var imgName))
                    {
                        ...
                        ((MediaAction)toAdd).MediaName = imgName;
                    }
                    else if (mes.VideoId != null && videos.TryGetValue((int)mes.VideoId, out var vidName))
                    {
                        ((MediaAction)toAdd).IsSticker = false;
                        ((MediaAction)toAdd).MediaName = vidName;
                    }
                    else if (mes.GifId != null && gifs.TryGetValue((int)mes.GifId, out var gifName))
                    {
                        ((MediaAction)toAdd).IsSticker = false;
                        ((MediaAction)toAdd).MediaName = gifName;
                    }
                    else if (mes.StickerId != null && stickers.TryGetValue((int)mes.StickerId, out var stickerName))
                    {
                        ((MediaAction)toAdd).IsSticker = true;
                        ((MediaAction)toAdd).MediaName = stickerName;
                    }
                    ...
                    {
                        share.SharedName = shareModel.Name;
                        if (shareModel.UserId != null && shareUsers.TryGetValue((int)shareModel.UserId, out var shareUser))
                        {
                            share.SharedUser = GetUserById((int)shareModel.UserId); // оставим пока
                        }
                    }
                    ...
                    if (toAdd is StaticMessage statMessage)
                    {
                        ...
                        statMessage.DelType = mes.ChangedAutoDelId != null && autoDelTypes.TryGetValue((int)mes.ChangedAutoDelId, out var delType)
                            ? GetAutoDelTypeById(mes.ChangedAutoDelId) // покажи этот метод потом
                            : null;
        ...
        private static List<Message> GetPinnedMessages(int chatId)
        {
            List<Message> res = new List<Message>();
            using (var model = new TelegramModel())
            {
                List<Messages> messes = model.Messages
                    .Where(x => x.ChatId == chatId && (bool)x.IsPinned)
                    .ToList();

                if (!messes.Any()) return res;

                var imageIds = messes.Where(x => x.ImageId != null).Select(x => (int)x.ImageId).ToHashSet();
                var videoIds = messes.Where(x => x.VideoId != null).Select(x => (int)x.VideoId).ToHashSet();
                var gifIds = messes.Where(x => x.GifId != null).Select(x => (int)x.GifId).ToHashSet();
                var stickerIds = messes.Where(x => x.StickerId != null).Select(x => (int)x.StickerId).ToHashSet();
                var shareIds = messes.Where(x => x.ShareContactId != null).Select(x => (int)x.ShareContactId).ToHashSet();
                var autoDelIds = messes.Where(x => x.ChangedAutoDelId != null).Select(x => (int)x.ChangedAutoDelId).ToHashSet();
                ...
                foreach (var mes in messes)
                {
                        ...
                        && shares.TryGetValue((int)mes.ShareContactId, out var shareModel))
                    {
                        ...
                        if (shareModel.UserId != null && shareUsers.TryGetValue((int)shareModel.UserId, out var shareUser))
                        {
                            share.SharedUser = new mainClass.User
                            {
                                Id = shareUser.Id,
                                Name = shareUser.Name,
                                Login = shareUser.Login
                            };
                        }
                    }
                    ...
                    if (toAdd is StaticMessage statMessage)
                    {
                        statMessage.MessageReferenceId = mes.MessageRefference ?? -1;
                        statMessage.DelType = mes.ChangedAutoDelId != null && autoDelTypes.TryGetValue((int)mes.ChangedAutoDelId, out var delType)
                            ? GetAutoDelTypeByTypeString(delType.Name)
                            : (Enums.Chat.AutoDeleteType?)null;
                        statMessage.Date = mes.StatDate;
                    }
        ...
        public static mainClass.User GetUserById(int userId)
        {
            using (var model = new TelegramModel())
            {
                var user = model.User.FirstOrDefault(x => x.Id == userId);
                if (user is null) return null;

                var color = model.UserColor.FirstOrDefault(x => x.UserId == userId);
                var images = model.UserImage
                ...
                mainClass.User res = new mainClass.User
                {
                    ...
                    LastSeenOnline = user.LastOnline is null ? DateTime.Now : (DateTime)user.LastOnline,
                    MainColor = color is null
                        ? new ColorHelper(userId)
                        : new ColorHelper(userId, (byte)color.R, (byte)color.G, (byte)color.B),
                    UserImages = images.Select(img => new mainClass.UserParams.UserImage
                    {
                        Name = img.Name,
                        Date = (DateTime)img.AddDate
                    }).ToList()
                };
        ...
        private static List<TelegramLib.MainClasses.User> GetBlockedContactsByUserId(int userId)
        {
            using (var model = new TelegramModel())
            {
                var blockedIds = model.BlockedContacts
                    .Where(x => x.UserId == userId)
                    .Select(x => (int)x.BlockedContactId)
                    .ToList();

                if (!blockedIds.Any()) return new List<mainClass.User>();

                var users = model.User
                    .Where(x => blockedIds.Contains(x.Id))
                    .ToList();

                var colorIds = users.Select(x => x.Id).ToList();
                var colors = model.UserColor
                    .Where(x => colorIds.Contains((int)x.UserId))
                    .ToDictionary(x => (int)x.UserId);

                var images = model.UserImage
                    .Where(x => colorIds.Contains((int)x.UserId))
                    .GroupBy(x => x.UserId)
                    .ToDictionary(g => (int)g.Key, g => g.OrderByDescending(i => i.AddDate).ToList());

                return users.Select(user => new mainClass.User
                {
                    Id = user.Id,
                    IsOnline = (bool)user.IsOnline,
                    Login = user.Login,
                    ...
                    PhoneNumber = user.PhoneNumber,
                    BirthDay = user.Birthday,
                    LastSeenOnline = user.LastOnline ?? DateTime.Now,
                    MainColor = colors.TryGetValue(user.Id, out var c)
                        ? new ColorHelper(user.Id, (byte)c.R, (byte)c.G, (byte)c.B)
                        : new ColorHelper(user.Id),
                    UserImages = images.TryGetValue(user.Id, out var imgs)
                        ? imgs.Select(i => new mainClass.UserParams.UserImage { Name = i.Name, Date = (DateTime)i.AddDate }).ToList()
                        : new List<mainClass.UserParams.UserImage>()
                }).ToList();
            }
        ...
        public static UserSettings.SettingsTypes.ChatSettings GetChatSettingsBySettingsId(int settingsId, int userId)
        {
            ...
            using (var model = new TelegramModel())
            {
                ...
                res.Wallpaper = GetChatWallpaperByChatSettingsId(res.Id);
                res.PossibleWallpapers = GetPossibleWallpapersForChatSetting(settingsId); 
            }
            return res;
        }
        ...
        {
            const int baseVal = 1;
            const string baseFont = "Time New Roman";
            ...
            using (var model = new TelegramModel())
            {
                ...
                settings.ThemeId = baseVal;
                settings.UserColorId = baseVal;
                settings.AutoNightId = baseVal;
                settings.Font = baseFont;
                settings.BgName = baseVal; ///
        ...
        public static void UpdateNotificationSoundsSettings(NotificationSettings newSettings)
        {
            using (var model = new TelegramModel())
            {
        ...
        private static void AddPhoneNumberSetting()
        {
            using (var model = new TelegramModel())
            {
                ...
                settings.WhoSeeId = _baseVal;
                settings.WhoCanFindNumber = _baseVal;
        ...
        {
            const string baseSeeStirng = "Everybody";
            using (var model = new TelegramModel())
            {
                foreach (var type in model.AwayForType)
                {
        ...
        private static List<mainClass.UserParams.UserImage> GetUserImagesByUserId(int userId)
        {
            ...
            using (var model = new TelegramModel())
            {
                ...
                return images.Select(img => new mainClass.UserParams.UserImage
                {
                    Name = img.Name,
                    Date = (DateTime)img.AddDate
                }).ToList();
        ...
        private static ColorHelper GetUserColorByUserId(int userId)
        {
            ...
            {
                var color = model.UserColor.FirstOrDefault(x => x.UserId == userId);
                if (color is null) return new ColorHelper(userId);
        ...
        private static ThemeType GetThemeById(int themeId)
        {
            using (var model = new TelegramModel())
            {
                ...
                for (int i = _baseVal; i <= (int)ThemeType.Night; i++)
                {
        ...
        private static AutoNightMode GetNightModeById(int nightModeId)
        {
            ...
            using (var model = new TelegramModel())
            {
                ...
                for (int i = minIndex; i <= (int)AutoNightMode.System; i++)
                {
        ...
        private static int GetChatImageIdByName(string imgName)
        {
            using (var model = new TelegramModel())
            {
                ...
                if (img is null) return _baseVal;
                return img.Id;
            }
        }
        ...
        public static int GetChatGifIdByName(string name)
        {
            using (var model = new TelegramModel())
            {
                ...
                if (gif is null) return _baseVal;
                return gif.Id;
            }
        }
        ...
        private static string NormalizePhone(string phone) => phone?.Replace("+", "") ?? string.Empty;

        public static string GetPhoneNumberFromUser()
        {
            const int checkId = 1003;
            using (var model = new TelegramModel())
            {
                Models.User user = model.User.FirstOrDefault(x => x.Id == checkId);
            ...
            int userId)
        {
            ...
            using (var model = new TelegramModel())
            {
                ...
                model.UserImage img =
                    model.UserImage.FirstOrDefault(x =>  x.UserId == userId && x.Name == toRemove.Name);
        ...
        {
            const int baseNotifyId = 5;
            using (var model = new TelegramModel())
            {
                MonitorNotifs notif = model.MonitorNotifs.FirstOrDefault(x => x.UserId == userId);

                return notif is null ? (NotifMessageSide.BottomRight, baseNotifyId) :
        ...
        {
            const int baseVolume = 100;
            const int baseSoundId = 2;
            using (var model = new TelegramModel())
            {
                ...
                sound.ChosenSoundId = baseSoundId;
                sound.UserId = userId;
        ...
        {
            const string baseSoundText = "Default";
            using (var model = new TelegramModel())
            {
                Sounds sound = model.Sounds.FirstOrDefault(x => x.Id == soundId);
                return sound is null ? baseSoundText : sound.Name;
            }
        }
        ...
        private static SoundSettings GetUserSoundByUserId(int userId)
        {
            ...
            const string defaultSound = "Default.mp3";
            ...
            using (var model = new TelegramModel())
            {
                ...
                res.ChosenSound = !(sound.IsDefaultSound is null) && (bool)sound.IsDefaultSound ? defaultSound : 
        ...
        private static TelegramLib.MainClasses.Messages.Message GetSavedMessageByMessages(Messages mes)
        {
            ...
            toAdd.Id = mes.Id;
            toAdd.RepliedQuote = mes.MessageQuote;
            toAdd.SenderUserId = mes.SenderId is null ? _baseVal : (int)mes.SenderId;
        ...
        public static bool IsChatOwnedByUser(int chatId, int userId)
        {
            using (var model = new TelegramModel())
            {
                return model.Chat.Any(x => x.Id == chatId && x.UserId == userId);
            }
        }
    }
}

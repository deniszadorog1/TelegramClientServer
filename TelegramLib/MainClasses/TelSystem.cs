...
namespace TelegramLib.MainClasses
{
    public class TelSystem
    {
        ...
        */
        ...
        public User GetMessageSender(int userId)
        {
            User user = GetUserById(userId);
            return user is null ? LoggedUser : user;
        }
        ...
        public UserChat GetChatByMessageId(int mesId)
        {
            ...
            if (SavedMesesChat.IsMessageContains(mesId)) return SavedMesesChat;
            return null;
        }
        ...
        public void RemoveAllMessagesFromChat(UserChat chat)
        {
            if (chat is null) return;
        ...
        public bool IsMessageIsInSchedById(int id)
        {
            for (int i = 0; i < Chats.Count; i++)
            {
                Messages.Message mes = Chats[i].ScheduleMessages.FirstOrDefault(x => x.Id == id);
                if (!(mes is null)) return true;
            }
            return false;
        }
...
/*            //Remove from replied
            ...
            TelegramLib.MainClasses.Messages.Message mes)
        {
            ...
            for (int i = 0; i < Chats.Count; i++)
            {
                if (Chats[i].IsMessageContains(mes) ||
                    Chats[i].IsInSchedMessages(mes)) return Chats[i];
            }
            return null;
        }
        ...
        {
            UserChat chat = GetChatByMessage(mes);
            return chat.GetPinnedMessageIndex(mes);
        }

        public Messages.Message GetNextPinnedMessage(
            Messages.Message tempMes)
        {
        ...
        public void EditMessage(Message toEdit)
        {
            ...
            {
                mes = Chats.Select(x => x.Messages.FirstOrDefault(y => y.Id == toEdit.Id)).FirstOrDefault();

                if (mes is null)
                {
                    mes = Chats.Select(x => x.ScheduleMessages.FirstOrDefault(y => y.Id == toEdit.Id)).FirstOrDefault();
                }
            }
            if (mes is null) return;
